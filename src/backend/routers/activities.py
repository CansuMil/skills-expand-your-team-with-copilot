...
def get_activities(
    ...
    - difficulty: Filter by difficulty level ('Beginner', 'Intermediate', 'Advanced', 'All' for activities without difficulty)
    """
    ...
    if difficulty:
        if difficulty == "All":
            # Show only activities that don't have a difficulty field
            query["difficulty"] = {"$exists": False}
        else:
            # Show activities with the specified difficulty
